...
function mostrarTablaHiperbola(x, y, decimal)
  ...
  for i = 1:(length(x))
   ...
   uicontrol (f, "style", "text", "string", num2str(totalY), "position",[290 (altura-(length(x)+1)*50) 90 30]);
   uicontrol (f, "style", "text", "string", num2str(totalCuadradoX), "position",[360 (altura-(length(x)+1)*50) 90 30]);
