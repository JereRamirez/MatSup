...
function mostrarTablaPotencial(x, y, decimal)
  ...
  for i = 1:(length(x)+1)
    if(i == (length(x)+1))
      ...
      uicontrol (f, "style", "text", "string", num2str(totalX), "position",[220 (altura-i*50) 90 30]);
      uicontrol (f, "style", "text", "string", num2str(totalXcuadrado), "position",[290 (altura-i*50) 90 30]);
      uicontrol (f, "style", "text", "string", num2str(totalY), "position",[360 (altura-i*50) 90 30]);
